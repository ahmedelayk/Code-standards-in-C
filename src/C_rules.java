...
public class C_rules extends CBaseVisitor<String[]> {
    ...
    C_rules(JSONObject declareObject, JSONObject globalObject, JSONObject localObject, String m){
    ...
    @Override public String[] visitGlobalDeclaration(CParser.GlobalDeclarationContext ctx) {
        ...
        if(numOfchilds == 2){
            ...
            if( type.length() > 4 &&type.substring(0,4).equals("enum") && !id.matches((String) globalObject.get("enum"))){
                System.out.println("error in file \""+ Filename + "\" line no. " + ctx.start.getLine() + ": variable name is " + "\"" + id + "\" and it should be like this pattern: " + (String) globalObject.get("enum"));
            } else if(type.length() > 5 && type.substring(0,5).equals("union") && !id.matches((String) globalObject.get("union"))){
                System.out.println("error in file \""+ Filename + "\" line no. " + ctx.start.getLine() + ": variable name is " + "\"" + id + "\" and it should be like this pattern: " + (String) globalObject.get("union"));
            } else if(type.length() > 6 && type.substring(0,6).equals("struct") && !id.matches((String) globalObject.get("struct"))){
                System.out.println("error in file \""+ Filename + "\" line no. " + ctx.start.getLine() + ": variable name is " + "\"" + id + "\" and it should be like this pattern: " + (String) globalObject.get("struct"));
            } else if(!id.matches((String) globalObject.get("other vars"))){
                System.out.println("error in file \""+ Filename + "\" line no. " + ctx.start.getLine() + ": variable name is " + "\"" + id + "\" and it should be like this pattern: " + (String) globalObject.get("other vars"));
            }
        ...
        //declaration of struct or union or enum
            ...
            if(!typeAndID[1].matches((String) declareObject.get(typeAndID[0]))){
                System.out.println("error in file \""+ Filename + "\" line no. " + ctx.start.getLine() + ": variable name is " + " \"" + typeAndID[1] + "\" and it should be like this pattern: " + (String) declareObject.get(typeAndID[0]));
            }
        }
    ...
    @Override public String[] visitLocalDeclaration(CParser.LocalDeclarationContext ctx) {
        ...
        if (numOfchilds == 2) {
            ...
            if (type.length() > 4 && type.substring(0,4).equals("enum") && !id.matches((String) localObject.get("enum"))) {
                System.out.println("error in file \""+ Filename + "\" line no. " + ctx.start.getLine() + ": variable name is " +  "\"" + id + "\" and it should be like this pattern: " + (String) localObject.get("enum"));
            } else if (type.length() > 5 && type.substring(0,5).equals("union") && !id.matches((String) localObject.get("union"))) {
                System.out.println("error in file \""+ Filename + "\" line no. " + ctx.start.getLine() + ": variable name is " +  "\"" + id + "\" and it should be like this pattern: " + (String) localObject.get("union"));
            } else if (type.length() > 6 && type.substring(0,6).equals("struct") && !id.matches((String) localObject.get("struct"))) {
                System.out.println("error in file \""+ Filename + "\" line no. " + ctx.start.getLine() + ": variable name is " + "\"" + id + "\" and it should be like this pattern: " + (String) localObject.get("struct"));
            } else if (!id.matches((String) localObject.get("other vars"))) {
                System.out.println("error in file \""+ Filename + "\" line no. " + ctx.start.getLine() + ": variable name is " + "\"" + id + "\" and it should be like this pattern: " + (String) localObject.get("other vars"));
            }
        }
    ...
    @Override public String[] visitExternalFunctionDefinition(CParser.ExternalFunctionDefinitionContext ctx) {
        ...
        if(!id.matches((String) declareObject.get("function"))){
            System.out.println("error in file \""+ Filename + "\" line no. " + ctx.start.getLine() + ": Function name is \"" + id + "\" and it should be like this pattern: " + (String) declareObject.get("function"));
        }
        visitChildren(ctx);
        String[] s = {ctx.getText()};
        return s;
